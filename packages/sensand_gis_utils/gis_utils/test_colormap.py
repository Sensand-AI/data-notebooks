...
class TestGetColormap(unittest.TestCase):
    ...
    def test_discrete_custom_levels(self):
        ...
        params = {'colormap': 'viridis'}
        custom_levels = [-1, 0, 1]
        result = get_colormap(params, custom_levels=custom_levels, color_count=21)
    ...
    def test_invalid_input(self):
        ...
        with self.assertRaises(ValueError):
            get_colormap(params, custom_levels=[], color_count=21)  # Expecting a ValueError for empty levels list
    ...
    def test_custom_levels_min_max_and_count(self):
        ...
        result = get_colormap(params, custom_levels=custom_levels, color_count=color_count)
        ...
        # Convert keys back to float for comparison (since they are stored as formatted strings)
        result_keys = list(map(float, result.keys()))
        
        # Check the minimum and maximum values
        self.assertAlmostEqual(min(result_keys), min(custom_levels), places=2, msg="The minimum level should match the minimum custom level")
        self.assertAlmostEqual(max(result_keys), max(custom_levels), places=2, msg="The maximum level should match the maximum custom level")
